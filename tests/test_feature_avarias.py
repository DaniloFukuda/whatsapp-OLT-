...
def carrinhas_operacionais(db_session, quantidade=1):
    service = PedidoService(db_session)
    pedido = service.criar(
        nome_cliente="Eloisa Integração",
        telefone_cliente="351933333333",
        data_planejada=datetime.now(timezone.utc),
        valor_global="300",
        pago=False,
        forma_pagamento=None,
        pedido_feito_por=PHONE,
        endereco_aproximado="Obra de integração",
        ponto_referencia=None,
        itens=[
            {
                "tipo_equipamento": TipoEquipamentoPedido.CARRINHA.value,
                "residuo_contratado": "Entulho Limpo",
                "horario_agendado": f"{9 + index:02d}:00",
            }
        ...
        [
            {
                ...
                "numero_adesivo": str(index),
                "fotos": [f"foto-chegada-{index}"],
            }
...
def test_partida_carrinha_false_conclui_com_foto_preserva_historico_e_financeiro(db_session, monkeypatch):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    service, pedido = carrinhas_operacionais(db_session)
    carrinha = pedido.contentores[0]
    carrinha.contentor_avariado = True
    carrinha.relato_avaria = "avaria historica da carrinha"
    ...
    service.confirmar_partida_carrinha(carrinha.id, PHONE, False, None, ["foto-partida"])
    db_session.expire_all()
    persistida = db_session.get(PedidoContentor, carrinha.id)
    pedido_db = service.get(pedido.id)
    assert persistida.status_operacional_carrinha == StatusOperacionalCarrinha.AGUARDANDO_DESPEJO.value
    assert persistida.partida_carrinha_feita_por == PHONE
    assert persistida.partida_carrinha_data_hora is not None
    assert persistida.contentor_avariado is True
    assert persistida.relato_avaria == "avaria historica da carrinha"
    assert persistida.status_resolucao_avaria == StatusResolucaoPedido.PENDENTE.value
    assert (Decimal(str(pedido_db.valor_global)), pedido_db.status_pagamento, pedido_db.forma_pagamento) == financeiro
    assert db_session.query(ContentorFoto).filter_by(
        pedido_contentor_id=persistida.id, tipo_foto=TipoFoto.RECOLHA.value
    ).count() == 1


@pytest.mark.parametrize(
    ("avariado", "relato"),
    [(True, "porta lateral muito danificada"), (False, "relato injetado diretamente")],
)
def test_servico_partida_false_rejeita_dados_de_avaria_antes_de_mutacao(
    db_session, monkeypatch, avariado, relato
):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    service, pedido = carrinhas_operacionais(db_session)
    carrinha = pedido.contentores[0]
    with pytest.raises(ValueError, match="não está disponível"):
        service.confirmar_partida_carrinha(carrinha.id, PHONE, avariado, relato, ["foto-partida"])
    db_session.expire_all()
    persistida = db_session.get(PedidoContentor, carrinha.id)
...
def test_partida_false_mantem_foto_obrigatoria(db_session, monkeypatch):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    service, pedido = carrinhas_operacionais(db_session)
    with pytest.raises(ValueError, match="foto da partida"):
        service.confirmar_partida_carrinha(pedido.contentores[0].id, PHONE, False, None, [])


def test_fluxo_whatsapp_partida_false_pula_pergunta_relato_e_exige_confirmacao(db_session, monkeypatch):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    _, pedido = carrinhas_operacionais(db_session)
    carrinha = pedido.contentores[0]
    conversa = ConversaWhatsApp(
        telefone=PHONE,
        estado_atual="v24_recolha_foto_acao",
        contexto_json={
            ...
            "recolhas": [],
            "fotos_recolha": ["foto-partida-operacional"],
        },
    )
    db_session.add(conversa)
    db_session.commit()
    agent = PedidoV24Agent(db_session)
    revisao = agent.handle(conversa, message("2"))
    assert conversa.estado_atual == "v24_recolha_confirmacao"
    assert "avaria" not in revisao.lower() and "relato" not in revisao.lower()
    assert carrinha.status_operacional_carrinha == StatusOperacionalCarrinha.EM_ATENDIMENTO.value
    final = agent.handle(conversa, message("1"))
    db_session.refresh(carrinha)
    assert "aguardam despejo" in final
    assert conversa.estado_atual == "idle"
...
@pytest.mark.parametrize(
    "state",
    ["v24_recolha_avaria", "v24_recolha_relato", "v24_recolha_confirmacao"],
)
def test_mudanca_para_false_durante_partida_recupera_revisao_sem_avaria(
    ...
    conversa = ConversaWhatsApp(
        ...
        contexto_json={
            ...
            "contentor_id": carrinha.id,
            "recolhas": [],
            "fotos_recolha": ["foto-partida-preservada"],
            "avariado": True,
            "relato_avaria": "relato parcial descartado",
        },
    )
    db_session.add(conversa)
    db_session.commit()
    resposta = PedidoV24Agent(db_session).handle(conversa, message("1"))
    db_session.refresh(carrinha)
    assert resposta.startswith(AVARIAS_DISABLED_MESSAGE)
    assert conversa.estado_atual == "v24_recolha_confirmacao"
    assert conversa.contexto_json["contentor_id"] == carrinha.id
    assert conversa.contexto_json["fotos_recolha"] == ["foto-partida-preservada"]
    assert "avariado" not in conversa.contexto_json and "relato_avaria" not in conversa.contexto_json
    assert carrinha.status_operacional_carrinha == StatusOperacionalCarrinha.EM_ATENDIMENTO.value
    assert carrinha.partida_carrinha_data_hora is None


def test_rollback_partida_false_permanece_integral(db_session, monkeypatch):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    service, pedido = carrinhas_operacionais(db_session)
    carrinha = pedido.contentores[0]
    erro = SQLAlchemyError("falha controlada")
    rollback_real = db_session.rollback
    rollbacks = []
    ...
    monkeypatch.setattr(db_session, "commit", falhar_commit)
    monkeypatch.setattr(db_session, "rollback", registrar_rollback)
    with pytest.raises(SQLAlchemyError) as capturada:
        service.confirmar_partida_carrinha(carrinha.id, PHONE, False, None, ["foto-partida"])
    assert capturada.value is erro and rollbacks == [True]
    SessionLocal = sessionmaker(bind=db_session.get_bind())
    with SessionLocal() as verificacao:
        persistida = verificacao.get(PedidoContentor, carrinha.id)
        assert persistida.status_operacional_carrinha == StatusOperacionalCarrinha.EM_ATENDIMENTO.value
        assert persistida.partida_carrinha_data_hora is None
...
def test_partida_true_preserva_criacao_de_avaria(db_session, monkeypatch):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "true")
    get_settings.cache_clear()
    service, pedido = carrinhas_operacionais(db_session)
    carrinha = pedido.contentores[0]
    service.confirmar_partida_carrinha(
        carrinha.id, PHONE, True, "porta lateral muito danificada", ["foto-partida"]
    )
    db_session.refresh(carrinha)
    assert carrinha.status_operacional_carrinha == StatusOperacionalCarrinha.AGUARDANDO_DESPEJO.value
...
def test_ciclo_completo_carrinha_false_mantem_300_pendente_e_isola_duas_carrinhas(db_session, monkeypatch):
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    service, pedido = carrinhas_operacionais(db_session, quantidade=2)
    primeira, segunda = pedido.contentores
    service.confirmar_partida_carrinha(primeira.id, PHONE, False, None, ["partida-1"])
    ...
    service.confirmar_despejo_carrinha(
        primeira.id, "Entulho Limpo", False, None, PHONE, ["foto-despejo"]
    )
    db_session.refresh(primeira)
    db_session.refresh(pedido)
    assert primeira.status_operacional_carrinha == StatusOperacionalCarrinha.CONCLUIDA.value
    assert segunda.status_operacional_carrinha == StatusOperacionalCarrinha.EM_ATENDIMENTO.value
    assert Decimal(str(pedido.valor_global)) == Decimal("300.00")
    assert pedido.status_pagamento == StatusPagamento.PENDENTE.value


def test_painel_false_esconde_avaria_mas_mantem_carrinha_e_true_reexibe(db_session, monkeypatch):
    _, pedido = carrinhas_operacionais(db_session)
    carrinha = pedido.contentores[0]
    carrinha.contentor_avariado = True
    carrinha.relato_avaria = "avaria visivel somente com flag ativa"
    carrinha.status_resolucao_avaria = StatusResolucaoPedido.PENDENTE.value
    db_session.commit()
    carrinha.partida_prevista_carrinha_data_hora = datetime.now(timezone.utc) + timedelta(hours=2)
    router = WhatsappRouterAgent(db_session)
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "false")
    get_settings.cache_clear()
    oculto = router._painel_v4(PerfilOperador.GESTOR)
    assert "Carrinha" in oculto
    assert "avaria visivel" not in oculto and "resolver avaria" not in oculto
    monkeypatch.setenv("FEATURE_AVARIAS_ENABLED", "true")
    get_settings.cache_clear()
    visivel = router._painel_v4(PerfilOperador.GESTOR)
    assert "Carrinha" in visivel
    assert "avaria visivel somente" in visivel
